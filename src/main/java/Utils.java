...
public class Utils extends BasePage {
    ...
    public static void captureScreenshot(String screenShotName) {
        ...
        try {
            //FileHandler.copy(source, new File("ScreenshotsTS\\" + screenShotName +timeStamp()+ ".png"));
...
//  wait1.until(ExpectedConditions.invisibilityOfElementLocated(By.id("register-button")));
    ...
    public String hoverColor(By by) {
        WebElement element = driver.findElement(by);
        String styleColor = element.getCssValue("color");
//        System.out.println(styleColor);
        String hex = Color.fromString(styleColor).asHex();
        System.out.println(hex);
        return hex;
    }

    public void hoverOnComputersCategory(By by) {
        WebElement computers = driver.findElement(by);
        Actions builder = new Actions(driver);
        Action mouseOverComputers = builder.moveToElement(computers).build();
        mouseOverComputers.perform();
    }
}
